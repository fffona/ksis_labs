...
BLOCKED_HOSTS = [
    ...
    'static.debank.com',
    'sanity-proxy-v2.phantom.app',
    'cloudflare-dns.com',
    'authentication.urban-vpn.com',
    'anti-phishing-protection.urban-vpn.com',
    'analytics.urban-vpn.com',
    'blowfish-blocklist-proxy.phantom.app',
    'ka-p.fontawesome.com:443ka-p.fontawesome.com',
...
def parse_request(data):
    try:
        if not data:  # игнор пустых запросов
...
def handle_client(client_socket):
    try:
        request_data = client_socket.recv(4096) # получаем запрос от клиента

        method, url, host, protocol, request_data = parse_request(request_data)
        if not method or not host:
            return

        host_name = host.split(':')[0]  # извлекаем домен без порта для проверки черного списка
        ...
        if request_lines:
            ...
            if not path:
                path = '/'
            request_lines[0] = f"{method} {path} {protocol}" # приводим заголовок к нормальному виду
                ...
                if not headers_found:
                    try:
                        ...
                        if 'http' in response_str.lower():
                            headers_found = True
                            status_match = re.search(r'HTTP/\d\.\d\s+(\d+\s+[^\r\n]*)', response_str) # поиск подходящего http ответа
                            if status_match:
...
def main():
    ...
    proxy_socket.listen(5) # макс кол-во подключений на сервере
